using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.EventSystems;

public class JoyStick : MonoBehaviour
{
    public GameObject Player;
    public Transform Stick;         // 조이스틱.
    public float speed = 5f;

    private Vector3 StickFirstPos;  // 조이스틱의 처음 위치.
    private Vector3 JoyVec;         // 조이스틱의 벡터(방향)
    private float Radius;           // 조이스틱 배경의 반 지름.
    private bool MoveFlag;

    void Start()
    {
        Radius = GetComponent<RectTransform>().sizeDelta.x * .4f;
        StickFirstPos = Stick.transform.position;

        // 캔버스 크기에대한 반지름 조절.
        float Can = transform.parent.GetComponent<RectTransform>().localScale.x;
        Radius *= Can;

        MoveFlag = false;
    }

    private void Update()
    {
        Vector3 moveVelocity = new Vector3();
        if (MoveFlag && Time.timeScale == 1)
        {
            if (JoyVec.x < 0)
            {
                Player.transform.localScale = new Vector3(0.22f, 0.22f, 1);
                moveVelocity = Vector3.left;
            }
            else
            {
                Player.transform.localScale = new Vector3(-0.22f, 0.22f, 1);
                moveVelocity = Vector3.right;
            }
            //            PlayerVec = new Vector2(JoyVec.x, 0);
            //            Player.transform.Translate(PlayerVec * Time.deltaTime * speed);
            Player.transform.position += moveVelocity * speed * Time.deltaTime;
        }
    }

    public void Drag(BaseEventData _Data)
    {
        PointerEventData Data = _Data as PointerEventData;
        Vector3 Pos = Data.position;

        // 조이스틱을 이동시킬 방향을 구함.(오른쪽,왼쪽,위,아래)
        JoyVec = (Pos - StickFirstPos).normalized;

        // 조이스틱의 처음 위치와 현재 내가 터치하고있는 위치의 거리를 구한다.
        float Dis = Vector3.Distance(Pos, StickFirstPos);

        Vector2 StickPos = Stick.position;

        // 거리가 반지름보다 작으면 조이스틱을 현재 터치하고 있는곳으로 이동. 
        if (Dis < Radius)
            Stick.position = new Vector2( StickFirstPos.x + JoyVec.x * Dis, StickFirstPos.y);
        // 거리가 반지름보다 커지면 조이스틱을 반지름의 크기만큼만 이동.
        else
            Stick.position = new Vector2(StickFirstPos.x + JoyVec.x * Radius, StickFirstPos.y);

        //Stick.position = new Vector2(StickPos.x, StickFirstPos.y);

        MoveFlag = true;
    }

    // 드래그 끝.
    public void DragEnd()
    {
        Stick.position = StickFirstPos; // 스틱을 원래의 위치로.
        JoyVec = Vector3.zero;          // 방향을 0으로.
        MoveFlag = false;
    }
}